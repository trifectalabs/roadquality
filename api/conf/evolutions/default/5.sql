...
CREATE OR REPLACE FUNCTION shortest_distance_route(start_lon double precision, start_lat double precision, end_lon double precision, end_lat double precision, bounding_box integer)
  ...
  WITH start_road      AS (SELECT * from closest_road_to_point(start_lon, start_lat)),
       ...
       end_point       AS (SELECT ST_GeometryFromText('POINT('||end_lon||' '||end_lat||')',4326)),
        route           AS (SELECT r.seq, r.cost, ways.the_geom from pgr_trsp('
                          SELECT gid::integer as id, source::int4, target::int4, length_m::float8 as cost FROM ways
